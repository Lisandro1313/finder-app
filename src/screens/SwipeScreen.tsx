  ...
  return (
    <div style={sb.bar}>
      {STORIES.map(s => {
        ...
        return (
          <div key={s.id} style={sb.item} onClick={() => handleOpen(s)}>
    ...
    <div style={sv.overlay} onClick={onClose}>
      <div style={sv.card} onClick={e => e.stopPropagation()}>
        <div style={sv.progressBar}>
          <div style={{ ...sv.progressFill, width: `${progress}%` }} />
        </div>
        <div style={sv.header}>
        ...
        <div style={sv.footer}>
          <div style={sv.footerInput} onClick={onClose}>
            Enviar mensaje a {story.nombre}... 💬
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={fp.overlay} onClick={onClose}>
      <div style={fp.panel} onClick={e => e.stopPropagation()}>
        ...
        <div style={fp.seccion}>
          <div style={fp.secLabel}>
            <span>Edad</span>
            <span style={fp.valor}>{local.edadMin} – {local.edadMax} años</span>
          </div>
          <div style={fp.rangeRow}>
            <span style={fp.rangeNum}>18</span>
            <input type="range" min={18} max={local.edadMax - 1} value={local.edadMin}
              onChange={e => setLocal(l => ({ ...l, edadMin: +e.target.value }))}
              style={fp.slider} />
            <input type="range" min={local.edadMin + 1} max={60} value={local.edadMax}
              onChange={e => setLocal(l => ({ ...l, edadMax: +e.target.value }))}
              style={fp.slider} />
        ...
        <div style={fp.seccion}>
          <div style={fp.secLabel}>
            ...
            <span style={fp.valor}>{local.distanciaMax} km</span>
          </div>
          <input type="range" min={1} max={100} value={local.distanciaMax}
            onChange={e => setLocal(l => ({ ...l, distanciaMax: +e.target.value }))}
            style={{ ...fp.slider, width: '100%' }} />
        </div>
        ...
        <div style={fp.seccion}>
          ...
          <div style={fp.generoRow}>
            {(['todos', 'mujer', 'hombre', 'nb'] as const).map(g => (
              <button key={g}
                style={{ ...fp.generoBtn, ...(local.genero === g ? fp.generoBtnActivo : {}) }}
                onClick={() => setLocal(l => ({ ...l, genero: g }))}
              >
...
export default function SwipeScreen() {
  ...
  const cardTransform = anim === 'like' ? 'translateX(150%) rotate(25deg)'
    : anim === 'nope' ? 'translateX(-150%) rotate(-25deg)'
    : drag.dragging || drag.x !== 0 ? `translate(${drag.x}px,${drag.y * 0.3}px) rotate(${dragRot}deg)`
    : 'none';
    ...
    <div style={s.container}>
      ...
      <div style={s.topBar}>
        ...
        <div style={s.topActions}>
          ...
          <button style={{ ...s.filtrosBtn, borderColor: filtrosActivos > 0 ? '#FF4F6A' : '#444' }}
      ...
      {agotado ? (
        <div style={s.agotado}>
          ...
          <p style={{ color: '#aaa', marginBottom: 12, fontSize: 14 }}>
            {filtrosActivos > 0 ? 'Probá ampliar tus filtros' : 'Volvé más tarde o recargá'}
          </p>
          {filtrosActivos > 0 && (
            <button style={{ ...s.btnReload, background: 'transparent', border: '1px solid #FF4F6A', color: '#FF4F6A', marginBottom: 10 }}
              onClick={() => setShowFiltros(true)}>
              🎛 Cambiar filtros
            </button>
          )}
          <button style={s.btnReload} onClick={() => { setCola(filtrar(filtros)); setAgotado(false); }}>
            🔄 Ver de nuevo
          </button>
        </div>
      ) : perfil ? (
        <>
          <div style={s.stack}>
            ...
            >
              ...
              <div style={{ ...s.tagLike, opacity: anim === 'like' ? 1 : likeOpacity }}>LIKE 💚</div>
              ...
              <div style={s.info}>
                ...
                <div style={s.distancia}>📍 {perfil.distancia} km · {perfil.ciudad}</div>
                <div style={s.compatibilidad}>
                  <span style={s.compatPct}>{perfil.compatibilidad}%</span>
          ...
          <div style={s.botones}>
            <button style={{ ...s.btnNope, transform: nopeOpacity > 0.5 ? 'scale(1.15)' : 'scale(1)' }}
              onClick={() => swipe('nope')}>✗</button>
            <button style={s.btnStar}>⭐</button>
            <button style={{ ...s.btnLike, transform: likeOpacity > 0.5 ? 'scale(1.15)' : 'scale(1)' }}
              onClick={() => swipe('like')}>♥</button>
          </div>
        </>
      ) : null}
      ...
      {matchPopup && (
        <div style={s.matchOverlay} onClick={() => setMatchPopup(null)}>
          <div style={s.matchCard} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ color: '#ccc', margin: '0 0 20px', fontSize: 14 }}>Vos y {matchPopup.nombre} se gustaron</p>
...
const s: Record<string, React.CSSProperties> = {
  ...
  agotado: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#0d0d0d' },
